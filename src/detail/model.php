...
function model_detail($colisId, $userId): array
{
    try {
        ...
        $query = 'SELECT c.*, m.id_mouv, u.nom AS user_nom, u.prenom, u.email, u.role_user, u.cle_entreprise FROM colis AS c JOIN mouvement AS m ON c.id_colis = m.colis_id JOIN users as u ON u.id_users = m.user_id WHERE m.user_id = :user_id AND c.id_colis = :colis_id';
